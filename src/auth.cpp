...
// Public-domain SHA256 implementation (FIPS 180-4 compliant)
// Safe for production use - no external dependencies required
...
static std::string sha256(const std::string& input) {
    ...
    for (int i = 0; i < 8; i++) {
        for (int j = 0; j < 4; j++) {
            oss << std::hex << std::setw(2) << std::setfill('0') << ((state[i] >> (24 - j*8)) & 0xFF);
        }
    }
    return oss.str();
}
...
/**
 * Destructor for the Auth class.
 */
Auth::~Auth() {}

/**
 * Generates a hash for the given password using a standards-compliant SHA256 implementation.
 * This implementation is public-domain (FIPS 180-4) and does not require external dependencies.
 *
...
bool Auth::verifyPassword(const std::string& password, const std::string& hash) {
    std::string generatedHash = generateHash(password);
